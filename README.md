...
2. The script will output the XML file in the `<SOURCE_FOLDER>/<TEAM_NAME>/levels/<CHARACTER>` folder, which has the same structure as the source folder. A file `xml_log_<DATE_TIME>.txt` will be created in the `<SOURCE_FOLDER>/logs` folder.
